...
angular.module("mm.foundation.mediaQueries", [])
	...
	.factory('mediaQueries', ['$document', 'matchMedia', function($document, matchMedia) {
		...
		var regex = /^[\/\\'"]+|(;\s?})+|[\/\\'"]+$/g;
		var queries = {
			topbar: getComputedStyle(head[0].querySelector('meta.foundation-mq-topbar'))
				.fontFamily.replace(regex, ''),
			small: getComputedStyle(head[0].querySelector('meta.foundation-mq-small'))
				.fontFamily.replace(regex, ''),
			medium: getComputedStyle(head[0].querySelector('meta.foundation-mq-medium'))
				.fontFamily.replace(regex, ''),
			large: getComputedStyle(head[0].querySelector('meta.foundation-mq-large'))
				.fontFamily.replace(regex, '')
		};

		return {
			topbarBreakpoint: function() {
				return !matchMedia(queries.topbar)
					.matches;
			},
			small: function() {
				return matchMedia(queries.small)
					.matches;
			},
			medium: function() {
				return matchMedia(queries.medium)
					.matches;
			},
			large: function() {
				return matchMedia(queries.large)
					.matches;
			}
		};
	}]);
		...
		return {
			...
			link: function(scope, element, attrs, controller) {
				...
				var onClick = function(event) {
					var dropdown = angular.element($document[0].querySelector(attrs.dropdownToggle));
					...
					if (!elementWasOpen && !element.hasClass('disabled') && !element.prop('disabled')) {
						...
						var getPosition = function() {
							...
							if (dynamic_pos) {
								...
								var fontSize = window.getComputedStyle(element[0])
		...
		return {
			...
			link: function(scope, element, attrs) {
				...
				$timeout(function() {
					...
					if (element[0].querySelectorAll('[autofocus]')
						.length > 0) {
	...
	.factory('$modalStack', ['$window', '$transition', '$timeout', '$document', '$compile', '$rootScope', '$$stackedMap',
		function($window, $transition, $timeout, $document, $compile, $rootScope, $$stackedMap) {
			...
			$modalStack.open = function(modalInstance, modal) {
				...
				body.append(faux[0]);
				var marginTop = parseInt(getComputedStyle(faux[0])
					.top) || 0;
				faux.remove();
				...
				function($injector, $rootScope, $q, $http, $templateCache, $controller, $modalStack) {
					...
					$modal.open = function(modalOptions) {
						...
						templateAndResolvePromise.then(function resolveSuccess(tplAndVars) {
							...
							var modalScope = (modalOptions.scope || $rootScope)
								.$new();
...
angular.module('mm.foundation.pagination', [])
	...
	.controller('PaginationController', ['$scope', '$attrs', '$parse', '$interpolate', function($scope, $attrs, $parse, $interpolate) {
		var self = this,
			setNumPages = $attrs.numPages ? $parse($attrs.numPages)
			.assign : angular.noop;
	...
	.factory('closest', [function() {
		return function(el, selector) {
			var matchesSelector = function(node, selector) {
				var nodes = (node.parentNode || node.document)
		...
		function($timeout, $compile, $window, $document, mediaQueries) {
			return {
				...
				controller: ['$window', '$scope', 'closest', function($window, $scope, closest) {
					...
					var dir = /rtl/i.test($document.find('html')
						.attr('dir')) ? 'right' : 'left';
				...
				link: function(scope, element, attrs) {
					...
					var onResize = function() {
						...
						angular.forEach(sections, function(section) {
							angular.element(section.querySelectorAll('li.moved'))
								.removeClass('moved');
						});
						$timeout(function() {
							scope.$apply();
					...
					if (topbarContainer.hasClass('fixed') || isSticky()) {
						...
						var stickyoffset = topbarContainer[0].getBoundingClientRect()
					...
					scope.$on('$destroy', function() {
						angular.element($window)
							.unbind('scroll', onResize);
						angular.element($window)
							.unbind('resize', onScroll);
					});
	...
	.directive('topBarSection', ['$compile', 'closest', function($compile, closest) {
		return {
			...
			link: function(scope, element, attrs, topBar) {
				...
				scope.reset = function() {
					angular.element(section[0].querySelectorAll('li.moved'))
						.removeClass('moved');
				};
		...
		function($compile, $parse, $q, $timeout, $document, $position, typeaheadParser) {
			...
			return {
				...
				link: function(originalScope, element, attrs, modelCtrl) {
					...
					var isLoadingSetter = $parse(attrs.typeaheadLoading)
						.assign || angular.noop;
					...
					var $setModelValue = $parse(attrs.ngModel)
						.assign;
...
angular.module("template/pagination/pager.html", [])
	.run(["$templateCache", function($templateCache) {
...
angular.module("template/progressbar/bar.html", [])
	.run(["$templateCache", function($templateCache) {
...
angular.module("template/progressbar/progress.html", [])
	.run(["$templateCache", function($templateCache) {
...
angular.module("template/progressbar/progressbar.html", [])
	.run(["$templateCache", function($templateCache) {
...
angular.module("template/rating/rating.html", [])
	.run(["$templateCache", function($templateCache) {
...
angular.module("template/typeahead/typeahead-match.html", [])
	.run(["$templateCache", function($templateCache) {
